...
本次实验开发的 Dice Roller 应用完全基于 Jetpack Compose 实现，不使用 XML 布局。整体采用组件化拆分设计，结构清晰。

- **入口**：MainActivity 的 onCreate 中设置主题，加载 DiceRollerScreen
- **主容器**：使用 Column 实现全屏、水平居中、垂直居中布局
- **组件拆分**：
  - DiceRollerScreen：管理状态与整体页面布局
  - DiceDisplayComponent：负责显示骰子图片与当前点数
  - RollDiceButtonComponent：提供掷骰子按钮，处理点击事件
